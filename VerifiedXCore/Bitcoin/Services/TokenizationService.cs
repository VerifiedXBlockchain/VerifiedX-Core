...
using static VerifiedXCore.Services.ArbiterService;
    ...
    public class TokenizationService
    {
        public static async Task<SmartContractMain?> CreateTokenizationScMain(string address, string fileLocation, string depositAddress, string proofJson, string tokenName = "vBTC Token", string description = "vBTC Token")
        {
            try
            {
                ...
                var nTx = new VerifiedXCore.Models.Transaction
                {
                ...
                nTx.Fee = VerifiedXCore.Services.FeeCalcService.CalculateTXFee(nTx);
                    ...
                    if (scState.SCStateTreiTokenizationTXes != null)
                    {
                        ...
                        if (balances.Any() || isOwner)
                        {
                            ...
                            if (good)
                            {
                                ...
                                var signature = VerifiedXCore.Services.SignatureService.CreateSignature(signatureInput + payload.ToAddress + payload.FromAddress, inputAccount.GetPrivKey, inputAccount.PublicKey);
        ...
        public static async Task<string> WithdrawalCoin(string vfxAddress, string btcToAddress, string scUID, decimal amount, long timestamp, string uniqueId, string signature, bool isTest, long chosenFeeRate = 10)
        {
            try
            {
                ...
                var sigCheck = VerifiedXCore.Services.SignatureService.VerifySignature(vfxAddress, message, signature);
        ...
        public static async Task<(VerifiedXCore.Models.Transaction?, string)> CreateTokenizedWithdrawal(TokenizedWithdrawals tw, string fromAddress, string toAddress, Account account, string scUID, bool isArb = false)
        {
            ...
            tx = new VerifiedXCore.Models.Transaction
            {
                Timestamp = TimeUtil.GetTime(),
            ...
            var txHash = tx.Hash;
            var signature = VerifiedXCore.Services.SignatureService.CreateSignature(txHash, account.GetPrivKey, account.PublicKey);
        ...
        private static async Task<(VerifiedXCore.Models.Transaction?, string)> CreateVFXTokenizedTransaction(string fromAddress, string toAddress, Account account, decimal amount, string scUID)
        {
            var scTx = new VerifiedXCore.Models.Transaction();
            var newSCInfo = new[]
            {
            ...
            scTx = new VerifiedXCore.Models.Transaction
            {
                Timestamp = TimeUtil.GetTime(),
                FromAddress = fromAddress,
                ToAddress = toAddress,
            ...
            scTx.Fee = VerifiedXCore.Services.FeeCalcService.CalculateTXFee(scTx);
        ...
        private static async Task<(VerifiedXCore.Models.Transaction?, string)> CreateVFXTokenizedTransactionMulti(BTCTokenizeTransactionMulti vbtcMulti, Account account, string signatureInput)
        {
            var scTx = new VerifiedXCore.Models.Transaction();
