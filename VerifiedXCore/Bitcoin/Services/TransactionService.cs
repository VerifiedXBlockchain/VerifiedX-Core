...
using VerifiedXCore.SecretSharing.Cryptography;
...
using System.Text;
using static VerifiedXCore.Models.Integrations;
        ...
        public static async Task<string> SendMultiSigTransactions(List<PubKey> pubKeys, decimal sendAmount, Account vfxAccount, string toAddress, string changeAddress, long chosenFeeRate, string scUID)
        {
            try
            {
                ...
                var sigData = new PostData.MultiSigSigningPostData
                {
                    ...
                    Timestamp = timestamp,
                    UniqueId = randomId,
                    Signature = VerifiedXCore.Services.SignatureService.CreateSignature($"{vfxAccount.Address}.{timestamp}.{randomId}", vfxAccount.GetPrivKey, vfxAccount.PublicKey),
                    Amount = sendAmount
                };
                ...
                List<VerifiedXCore.Models.Arbiter> randomArbs = myList.Take(Globals.TotalArbiterParties).ToList();
