        ...
        public static async Task Start()
        {
            try
            {
                if (Globals.IsFrostValidator)
                {
                    ...
                    _ = Task.Run(async () => await VerifiedXCore.Bitcoin.Services.FrostKeyBackupService.PeriodicRebroadcastLoop());
                    _ = Task.Run(async () => await VerifiedXCore.Bitcoin.Services.FrostKeyBackupService.StaleBackupCleanupLoop());
        ...
        private static async Task AutoRecoverFrostKeysIfNeeded()
        {
            try
            {
                ...
                if (localKeys.Count > 0)
                {
                    ...
                    var validators = VerifiedXCore.Bitcoin.Services.VBTCValidatorRegistry.GetActiveValidators();
                    if (validators != null && validators.Count > 1)
                    {
                        await VerifiedXCore.Bitcoin.Services.FrostKeyBackupService.BroadcastAllExistingBackups(
                ...
                var activeValidators = VerifiedXCore.Bitcoin.Services.VBTCValidatorRegistry.GetActiveValidators();
                ...
                var recoveredCount = await VerifiedXCore.Bitcoin.Services.FrostKeyBackupService.RecoverKeysFromPeers(
