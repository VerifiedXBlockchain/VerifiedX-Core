...
Same deployment pattern as **`Frost/`**: platform-specific binaries live under `win/`, `linux/`, and `mac/`, and `VerifiedXCore.csproj` copies them to the output directory with a flat `<Link>` name so `DllImport("plonk_ffi")` resolves.
